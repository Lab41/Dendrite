...
angular.module('dendrite.controllers', []).
        ...
        var pollActive = function() {
          ...
          Graph.get({graphId: $scope.graphId})
                .$then(function(dataGraph) {
                    Project.jobs({projectId: $routeParams.projectId})
    ...
    controller('VertexListCtrl', function($scope,$rootScope, $location, $timeout, $routeParams, $filter, $q, appConfig, User, Graph, Project, Vertex, Edge, ElasticSearch) {
    ...
    controller('VertexCreateCtrl', function($rootScope,$scope,$routeParams, $location, User, Vertex, ElasticSearch, $timeout, appConfig) {
        $scope.User = User;
        $scope.save = function() {
    ...
    controller('VertexEditCtrl', function($scope, $routeParams, $location, User, Vertex) {
        ...
        $scope.save = function() {
            Vertex.update({graphId: $scope.graphId, vertexId: $scope.vertexId}, $scope.vertex)
                  .$then(function(data) {
                    //TODO Should search for new object
                    ElasticSearch.verifyId($scope.graphId, data.data.results._id, "vertex");
                  });
        };
    }).
